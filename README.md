...
```powershell
Compress-Archive -Path index.html,poster.webp,assets,common,config -DestinationPath ../astranova-roomtalk.zip -Force
```

將 `astranova-roomtalk.zip` 上傳後台。ZIP 根目錄須直接包含 `index.html`、`poster.webp`、`assets/`、`common/`、`config/`，不可再包一層目錄。專案及 ZIP 名稱只保留遊戲名稱，不添加 noskin 或 html 後綴。
...
- RoomTalk 英文介面的部分對話選項仍混入中文；繁／簡中文各比英文少 2 個語系 key。
- 2026-09-16 已將 27 張正式 PNG 原尺寸轉為保留 alpha 的高品質 WebP，同步更新 manifest 與 JS 引用；本機 HTTP 子路徑載入無 console 警告或錯誤，重建 ZIP 由 46.28 MB 降至 23.40 MB。
